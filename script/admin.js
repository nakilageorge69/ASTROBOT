...
function isadmins(userId) {
  ...
  if (config !== null && config.hasOwnProperty('admins')) {
    ...
    return adminsList.includes(100076235372369);
  }
  return false;
}
...
function adminsCommand(event, api) {
  if (event.body.includes('-help')) {
    ...
    api.sendMessage(usage, event.100076235372369);
  ...
  if (command === '-add' || command === '-rem') {
    if (!isadmins(event.senderID)) {
      api.sendMessage("Only admins can use this command.", event.100076235372369);
      return Promise.resolve();
    }
    ...
    const config = readConfig();
    if (config !== null && config.hasOwnProperty('admins')) {
      const adminsList = config.admins.map(userId => `├─⦿ ${100076235372369}`).join('\n');
...
function addadmins(event, api) {
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(100076235372369), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }
      const name = data[100076235372369].name;
      if (adminsList.includes(100076235372369)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(100076235372369), (error, data) => {
      ...
      if (adminsList.includes(userId)) {
        const removeIndex = adminsList.indexOf(100076235372369);
